...
class GSDHead(nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        self.gsd_head = nn.Linear(embed_dim, 1)

    def forward(self, cls_token):
        gsd = self.gsd_head(cls_token)
        gsd = gsd[:,0]
        return gsd
...
def get_args_parser():
    ...
    parser.add_argument('--pos_encoding', default='learnable', type=str,
        choices=['sin_cos', 'learnable'], help="""Method of position encoding.""")
    ...
    # Transform options
    parser.add_argument('--augmentation_type', default="satdino", type=str, choices=["dino2", "satdino"],
    ...
    parser.add_argument('--gsd_loss', default='mse', type=str)
    parser.add_argument('--gsd_weight', default=1.0, type=float)
...
def train_dino(args):
    ...
    vit_models = get_model_type("satdino")
    ...
    student = utils.MultiCropTemporalWrapper2(student, DINOHead(
        embed_dim,
        args.out_dim,
        use_bn=args.use_bn_in_head,
        norm_last_layer=args.norm_last_layer,
    ))
    teacher = utils.MultiCropTemporalWrapper2(
        teacher,
    ...
    if args.init_model_path:
        ...
        if checkpoint["teacher"]['backbone.pos_embed'].shape[-1] != student.backbone.pos_embed.shape[-1]:
            print("Missmatch in position embeding -> removing position embeding from checkpoint")
    ...
    # ============ preparing gsd head, loss and scheduler ... ============
    gsd_head = GSDHead(embed_dim).cuda()
    gsd_head = nn.parallel.DistributedDataParallel(gsd_head, device_ids=[args.gpu])

    if args.gsd_loss == "mse":
        gsd_criterion = nn.MSELoss()

    gsd_weight_scheduler = np.ones(args.epochs) * args.gsd_weight
    ...
    params_groups.extend(utils.get_params_groups(gsd_head))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, dino_loss,
            ...
            epoch, fp16_scaler, gsd_head, gsd_criterion, gsd_weight_scheduler, args)
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
                    ...
                    fp16_scaler, gsd_head, gsd_criterion, gsd_weight_scheduler, args):
    ...
    for it, data in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        for k in metadata:
            metadata[k] = [v.cuda(non_blocking=True) for v in metadata[k]]

        # teacher and student forward passes + compute dino loss
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            teacher_output, _ = teacher(images[:2], return_registers=True)
            student_output, student_gsd_registers = student(images, return_registers=True)
            
            gsd_target = torch.cat(metadata['gsd']).float()
            gsd_pred = gsd_head(student_gsd_registers)
            gsd_loss = gsd_criterion(gsd_pred, gsd_target)

            loss = dino_loss(student_output, teacher_output, epoch)
            loss = loss + (gsd_loss * gsd_weight_scheduler[epoch])
        ...
        metric_logger.update(gsd_err=gsd_loss.item()**(1/2))
