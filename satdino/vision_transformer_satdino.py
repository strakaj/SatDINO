...
from satdino.utils import trunc_normal_, get_1d_sincos_pos_embed
...
class VisionTransformerSatDINO(nn.Module):
    """ Vision Transformer """
    ...
    def __init__(
        ...
        self.gsd_register = nn.Parameter(torch.zeros(1, 1, embed_dim))
        trunc_normal_(self.gsd_register, std=.02)

        # positional encoding
        if pos_encoding_method == "learnable":
            self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 2, embed_dim))
            trunc_normal_(self.pos_embed, std=.02)
        elif pos_encoding_method == "sin_cos":
            positions = torch.arange(num_patches + 2)
            self.pos_embed = get_1d_sincos_pos_embed(embed_dim, positions).unsqueeze(0).cuda()
        ...
        self.blocks = nn.ModuleList([Block(drop_path=dpr[i], **block_kwargs) for i in range(depth)])
...
def vit_tiny(patch_size=16, **kwargs):
    model = VisionTransformerSatDINO(
